...
- [ ] `/setup` は誰が・いつ・なぜ表示されるのか説明できるか
- [ ] `react-hook-form` と `zod` の役割分担を説明できるか（なぜ2つ使うのか）
- [ ] `zodResolver` は何をしているか
- [ ] `Popover` + `Calendar` の組み合わせで日付ピッカーを作る理由（ネイティブ `<input type="date">` との違い）
- [ ] モック段階でAPIを呼ばない設計の意図は何か
